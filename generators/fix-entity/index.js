...
module.exports = class extends BaseGenerator {
        ...
        return {
            ...
            readConfig() {
                ...
                if (this.force && !this.dbhIdName) {
                    throw new Error(
                        'You\'ve used option "--force" with an invalid configuration file, fix-entity stops his execution'
                    );
                }
    ...
    writing() {
        ...
        const replaceTableName = (paramFiles, newValue) => {
            ...
            this.replaceContent(
                paramFiles.ORM,
                `@Table(name = "${this.entityTableName}`,
                `@Table(name = "${newValue}`
            );
            this.replaceContent(
                paramFiles.liquibaseEntity,
                `<createTable tableName="${this.entityTableName}`,
                `<createTable tableName="${newValue}`
            );
        };
        ...
        const replaceIdName = (paramFiles, newValue) => {
            ...
            this.replaceContent(
                paramFiles.liquibaseEntity,
                `(<column name=")(id|${this.dbhIdName})(".*>\\s*<constraints primaryKey="true")`,
                `$1${newValue}$3`,
                true
            );
        };
        ...
        this.columnsInput.forEach((columnItem) => {
            ...
            this.replaceContent(
                files.ORM,
                `@Column\\(name = "(${columnItem.fieldNameAsDatabaseColumn}|${oldValue})"`,
            ...
            this.replaceContent(
                files.liquibaseEntity,
                `<dropDefaultValue tableName="${this.entityTableName}" columnName="(${columnItem.fieldNameAsDatabaseColumn}|${oldValue})"`,
                `<dropDefaultValue tableName="${this.entityTableName}" columnName="${newValue}"`,
                true
            );
        });
        ...
        this.relationshipsInput.forEach((relationshipItem) => {
            ...
            if (
                ...
                this.replaceContent(
                    files.liquibaseConstraints,
                    `baseTableName="${this.entityTableName}`,
                    `baseTableName="${this.tableNameInput}`
                );
                this.replaceContent(
                    files.liquibaseConstraints,
                    `(referencedColumnNames=")id("\\s*referencedTableName="${otherEntity.entityTableName}")`,
                    `$1${otherEntityIdName}$2`,
                    true
                );
                ...
                if (relationshipItem.relationshipType === 'many-to-one') {
                    ...
                    this.replaceContent(
                        files.ORM,
                        `(@JoinColumn.*\\))?(\\s*)(private ${relationshipItem.otherEntityNameCapitalized} ${relationshipItem.relationshipName};)`,
                        `$2@JoinColumn(name = "${newValue}")$2$3`,
                        true
                    );
                } else {
                    this.replaceContent(
                ...
                this.replaceContent(
                    files.liquibaseEntity,
                    `<addPrimaryKey columnNames="${otherEntityColumnName}, (${columnName}|${oldValue})`,
                    `<addPrimaryKey columnNames="${otherEntityNewValue}, ${newValue}`,
                    true
                );
                this.replaceContent(
                    files.liquibaseEntity,
                    `<column name="(${otherEntityColumnName}|${otherEntityOldValue})"`,
                    `<column name="${otherEntityNewValue}"`,
                    true
                );
                this.replaceContent(
                    files.liquibaseEntity,
                    `(tableName=")(${junctionTableJhipster}|${junctionTableOldValue})`,
                    `$1${junctionTableNewValue}`,
                    true
                );
                this.replaceContent(
                    files.ORM,
                    `(@JoinTable\\(name = ")(${junctionTableJhipster}|${junctionTableOldValue})`,
                    `$1${junctionTableNewValue}`,
                    true
                );
                this.replaceContent(
                    files.ORM,
                    `joinColumns = @JoinColumn\\(name="(${otherEntityColumnName}|${otherEntityOldValue})`,
                    `joinColumns = @JoinColumn(name="${otherEntityNewValue}`,
                    true
                );
                this.replaceContent(
                    files.ORM,
                    `(@JoinColumn\\(name="${otherEntityNewValue}", referencedColumnName=")(id|${this.dbhIdName})`,
                    `$1${this.idNameInput}`,
                    true
                );
                this.replaceContent(
                    files.ORM,
                    `inverseJoinColumns = @JoinColumn\\(name="(${columnName}|${oldValue})`,
                    `inverseJoinColumns = @JoinColumn(name="${newValue}`,
                    true
                );
                this.replaceContent(
                    files.ORM,
                ...
                this.replaceContent(
                    files.liquibaseConstraints,
                    `(baseTableName=")(${junctionTableJhipster}|${junctionTableOldValue})`,
                    `$1${junctionTableNewValue}`,
                    true
                );
                this.replaceContent(
                    files.liquibaseConstraints,
                    `<addForeignKeyConstraint baseColumnNames="(${otherEntityColumnName}|${otherEntityOldValue})`,
                    `<addForeignKeyConstraint baseColumnNames="${otherEntityNewValue}`,
                    true
                );
                this.replaceContent(
                    files.liquibaseConstraints,
                    `referencedTableName="${this.entityTableName}`,
                    `referencedTableName="${this.tableNameInput}`,
                    false
                );
                this.replaceContent(
                    files.liquibaseConstraints,
                    `(referencedColumnNames=")id("\\s*referencedTableName="${this.entityTableName}")`,
                    `$1${this.idNameInput}$2`,
                    true
                );
                this.replaceContent(
                    files.liquibaseConstraints,
            ...
            this.replaceContent(
                files.liquibaseEntity,
                `<column name="(${columnName}|${oldValue})"`,
                `<column name="${newValue}"`,
                true
            );
            this.replaceContent(
                files.liquibaseConstraints,
                `<addForeignKeyConstraint baseColumnNames="(${columnName}|${oldValue})`,
                `<addForeignKeyConstraint baseColumnNames="${newValue}`,
                true
            );
        });
    }
